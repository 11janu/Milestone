...
def item():
    ...
    if form.validate_on_submit():
        if form.id.data:  # it's an update
            try:
                result = DB.update(
                    "UPDATE IS601_S_Products set name = %s, description = %s, category = %s, stock = %s, unit_price = %s, visibility=%s WHERE id = %s",
                    form.name.data, form.description.data, form.category.data, form.stock.data, form.unit_price.data,
                    1 if form.visibility.data else 0, form.id.data)
                if result.status:
            ...
            try:
                ...
                VALUES (%s,%s,%s,%s,%s, %s)""",
                                   form.name.data, form.description.data, form.category.data, form.stock.data,
                                   form.unit_price.data, 1 if form.visibility.data else 0)
                if result.status:
    ...
    if id:
        try:
            result = DB.selectOne(
                "SELECT id, name, description, category, stock, unit_price, visibility FROM IS601_S_Products WHERE id = %s",
                id)
...
def items():
    ...
    try:
        result = DB.selectAll(
            "SELECT id, name, description, stock, unit_price, visibility FROM IS601_S_Products LIMIT 25", )
...
def purchase():
    ...
    try:
        ...
        if not has_error:
            # ucid: JS2679 date: 03/05/2023
            address = request.form.get('apt') + "," + request.form.get('city') + "," + request.form.get(
                'state') + "," + request.form.get('country') + "," + request.form.get('zpcode')
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s)""", total, current_user.get_id(), address, payment_method,
                                  money_received, fname, lname)
            if not result.status:
                flash("Error generating order", "danger")
                DB.getDB().rollback()
                has_error = True
            else:
                order_id = int(DB.selectAll("""SELECT LAST_INSERT_ID()""").rows[0]["LAST_INSERT_ID()"])
